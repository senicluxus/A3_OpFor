class CfgPatches
{
	class A3_Opf_Characters_F_Opf
	{
		author = $STR_A3_A_AveryTheKitty;
		name = "Arma 3 Opposing Forces - Characters and Clothing";
		url = linkOpf;
		requiredAddons[] =
		{
			A3_Characters_F_Enoch,
			A3_Opf_Data_F_Opf
		};
		requiredVersion = 0.1;
		units[] =
        {
            // Paramilitary
            Opf_O_P_medic_F,
            Opf_O_P_soldier_1_F,
            Opf_O_P_soldier_AR_F,
            Opf_O_P_soldier_exp_F,
            Opf_O_P_soldier_GL_F,
            Opf_O_P_soldier_LAT_F,
            Opf_O_P_soldier_M_F,
            Opf_O_P_soldier_TL_F,
            Opf_O_P_Soldier_unarmed_F,

            // Raven
            Opf_I_R_engineer_F,
            Opf_I_R_medic_F,
            Opf_I_R_soldier_F,
            Opf_I_R_soldier_GL_F,
            Opf_I_R_soldier_LAT_F,
            Opf_I_R_soldier_MG_F,
            Opf_I_R_soldier_M_F,
            Opf_I_R_soldier_TL_F,
            Opf_I_R_soldier_UAV_06_F,
            Opf_I_R_soldier_UAV_06_medical_F,
            Opf_I_R_Soldier_UAV_F,
            Opf_I_R_Soldier_unarmed_F,

            // Separatists
            Opf_O_S_Crew_F,
            Opf_O_S_Soldier_1_F,
            Opf_O_S_Soldier_2_F,
            Opf_O_S_Soldier_3_F,
            Opf_O_S_Soldier_4_F,
            Opf_O_S_Soldier_5_F,
            Opf_O_S_Soldier_6_F,
            Opf_O_S_Soldier_7_F,
            Opf_O_S_Soldier_8_F,
            Opf_O_S_Soldier_9_F,
            Opf_O_S_Soldier_Base_unarmed_F,

            // Insurgents
            Opf_I_I_Soldier_1_F,
            Opf_I_I_Soldier_2_F,
            Opf_I_I_Soldier_3_F,
            Opf_I_I_Soldier_4_F,
            Opf_I_I_Soldier_5_F,
            Opf_I_I_Soldier_6_F,
            Opf_I_I_Soldier_7_F,
            Opf_I_I_Soldier_8_F,
            Opf_I_I_Soldier_9_F,
            Opf_I_I_Soldier_Base_unarmed_F,
            Opf_I_I_tribal_scout,
            Opf_I_I_tribal_watcher,
            Opf_I_I_tribal_enforcer,
            Opf_I_I_tribal_hireling,
            Opf_I_I_tribal_medic,
            Opf_I_I_tribal_sapper,
            Opf_I_I_tribal_deserter,

            // Patriots
            Opf_I_P_Soldier_1_F,
            Opf_I_P_Soldier_2_F,
            Opf_I_P_Soldier_3_F,
            Opf_I_P_Soldier_4_F,
            Opf_I_P_Soldier_5_F,
            Opf_I_P_Soldier_6_F,
            Opf_I_P_Soldier_7_F,
            Opf_I_P_Soldier_8_F,
            Opf_I_P_Soldier_Base_unarmed_F
        };
		weapons[] = {/* Automated */};
	};
};